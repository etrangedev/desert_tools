module Desert
  module Rails
    module RouteSet
      ...
      def routes_from_plugin(name, options = {})
        name = name.to_s
        ...
        options_to_s = options.to_a.collect { |p| "#{p.first.inspect} => #{p.last.inspect}" }.join(', ') 
                
        code = %{
          with_options #{options_to_s} do |plugin|
            #{IO.read(routes_path)}
          end
        }
        
        eval(code, binding, routes_path) if File.file?(routes_path)
      end
    end
  end
end
